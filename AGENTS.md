...
- **Every PR workflow skips drafts, and that is now load-bearing rather than incidental** — a draft's empty check list is not a pass. `work-implement` opens its PRs as drafts on purpose and `work-review` marks one ready only once it would accept, so CI is spent once per _finished_ instead of once per push (**The draft gate** in `work-implement`'s REFERENCE). Each of the three therefore needs both halves: the draft gate _and_ `ready_for_review` in its trigger `types`, or the un-draft fires nothing.
