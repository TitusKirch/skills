...
**Ownership is the axis, not size.** A file looks monolithic when it hosts mechanics it does not own, and that is the defect worth fixing. [ADR-0020](/adr/0020-separate-installable-alone-from-runnable-alone) already settled the rule — a mirrored copy belongs where the mechanic is owned — and left the moves as follow-on work; those moves landed for `<skills-config>` and `<skills-worklock>`, which the two queue skills now read from their worker's reference rather than carrying.
...
Rejected: **deleting content to shrink the file** — [ADR-0014](/adr/0014-let-rationale-travel-with-the-skill) is why the rationale is there, and an agent that knows why a rule exists does not route around it.
