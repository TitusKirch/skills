...
The contract and the catalogue now agree by construction rather than by intention: the gap is still there, but it is enumerated, each entry says why, and it cannot widen silently.
...
Scoping a skill costs a permission prompt wherever the scoped list is wrong or incomplete. That is the correct failure mode and the reason the migration starts with human-attended skills — a prompt is an inconvenience there and a hang in an unattended loop, which is why the two queue skills are on the permanent side of the list rather than at the front of the queue.
