...
Refusing the exec primitives raises that cost on purpose: a skill that reached for `sh`, `find` or `sed` now prompts where a one-line grant would have silenced it. Taken across the remaining seventeen it also narrows what a migration is allowed to be — a skill whose real work runs through a shell cannot be scoped honestly and belongs on the named-exception list, saying so, rather than carrying a `Bash(sh:*)` that reads like progress.
