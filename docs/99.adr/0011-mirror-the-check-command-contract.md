...
What was shared was the **reading**: [ADR-0003](/adr/0003-mirror-shared-content-into-each-skill) mirrors the resolver into every skill, byte-identical and drift-checked. What was not shared was everything that follows it.
...
The check-command contract becomes the **third mirrored block**, alongside the config contract and the author-authority rule, sourced from `scripts/verify-block.md` and drift-checked by `pnpm skills:check`. Marker syntax follows [ADR-0010](/adr/0010-choose-marker-syntax-by-its-reader): a named element, because the body is instruction a model must act on.

Two variants, keyed by **the tree the command runs in** — the same criterion-not-roster shape [ADR-0004](/adr/0004-derive-author-authority-from-a-criterion) uses, so a skill added later is classified by what it does:
...
The cost is prompt length: five skills carry a block they previously stated in one line. That is the standing trade of [ADR-0003](/adr/0003-mirror-shared-content-into-each-skill) — the alternative is a link that resolves to nothing once the skill is installed.
