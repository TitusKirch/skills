...
The cost is a second shape to learn and a judgement call at its boundary — a skill with two ordered steps is full, and someone has to say so. **The criterion is prose, not a gate**: nothing in `test/` can measure "multi-step", so it holds in review or not at all. That is the same standing as the [naming rules](https://github.com/TitusKirch/skills/blob/main/skills/README.md#naming) and the reference rules beside it, and it is why the criterion is written as one sentence a reviewer can quote.
