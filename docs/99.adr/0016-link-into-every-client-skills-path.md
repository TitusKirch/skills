...
`test/opencode-compatibility.test.ts`, run by the pinned OpenCode workflow, checks that `skills:link` populates an isolated `HOME`. It then loads the published frontmatter through an isolated project catalogue, keeping OpenCode's `debug skill` response bounded while verifying every name and source location; both unattended queue-agent permission templates are checked through `debug agent`. The test is deliberately a separate compatibility workflow: the normal `pnpm verify` gate stays pnpm-only.
