...
stateDiagram-v2
    ...
    state "ai: working"           as working
    state "ai: review"            as review
    state "ai: changes requested" as changes
    state "ai: needs human"       as human
    state "ai: done"              as done
    state "ai: blocked"           as blocked

    ready   --> working : implement loop leases it
    changes --> working : implement loop leases it, re-work
    working --> review  : checks green, committed and PUSHED
    working --> blocked : checks red and unfixable

    review  --> done    : approved, low risk
    review  --> human   : approved but risky or sensitive
    review  --> changes : feedback, rounds 1-2
    review  --> human   : feedback, round 3 and beyond
    review  --> blocked : broken

    human   --> done    : human accepts
    human   --> changes : human wants changes
