...
- **`docs`** — `preset` from project type (cli / package / app / service / infra), or `false` to opt out; language inherits root. `docs.instructions` for repo-wide docs wording, only on an explicit preference. **`render` is the one key here that must never be inferred** — it names the rendering theme the repo publishes its `docs/` through, and the tree that most needs it is often the one carrying no trace of it (a `docs/` pulled into someone else's multi-repo site has no dependency to find). So detect nothing: write it only when the user states the target, and leave it out otherwise, where absent correctly means the portable default.
- **`work`** — only when the repo runs the queue. Pick the tracker (above); it, `linear.team` and `local.dir` fall back to `issue.*`. **On `local` the schema requires nothing alongside the tracker** — every key there has a default — so write the tracker and stop; the lifecycle `labels` are still read as **config keys** (they name the `state` field's values), while their strings go unused. Set `cap` (default 10), `branch` (`worktree` default), and the lifecycle `labels`. **On Linear the schema requires `linear.team`, `linear.statuses` (startable states) and `labels.repo` (the repo discriminator) — set all three.** Also offer `linear.states` — the workflow-state map. It has no default (state names are per-team), and without it the worker moves the label but never the board, so propose it from the team's real states rather than leaving it out silently. **Two of its keys need care.** `accepted` is where AI review lands an issue and is deliberately **not** a terminal column — the work is still on `pr.base`, so propose a `Ready for release` / `Accepted` state and put it in `statuses` as well, because a human hands an accepted issue back by relabelling it without moving the state. `done` is terminal-shipped and **no work skill writes it**: propose it only where something will — Linear's own integration when `pr.base` **is** the default branch, or the `release` skill when the repo runs it — and otherwise leave it unmapped rather than mapping a `Done` nothing observes. **Say what the mapping is worth while proposing it:** every state in it is a **best-effort write**, because Linear's GitHub integration writes that same field on any pull-request event, label-blind, and nothing in the skills puts it back. So the team turns that integration's state automation off **and** keeps `statuses` wide enough to hold every state an eligible issue legitimately sits in — a human moving a card writes the same field, so that much stays required either way, and otherwise a state the repo never chose silently drops issues out of the queue. Widening it **further**, to cover what the integration writes, is a separate call and worth it only where `labels.ready` is a real label: under `labels.ready: false` there is no label gate left, `statuses` becomes the sole positive one, and widening hands the queue to the foreign writer instead. **What that repo does instead** is silence the automation, or restore a positive label gate — **not** leave `linear.states` unmapped: that settles what the board claims and nothing about the queue, because `statuses` is a separate key the schema requires and the selection query reads either way. **`labels.needsTriage` is the one label here the loops only read, never write, and it is off by default** — propose it, with the repo's exact spelling, whenever the repo's issue templates declare an untriaged marker (`needs triage`, `triage`, `status: needs-triage`); it is the repo's own label, so it can only ever be pinned, never defaulted, and left unset the skills stay blind to it.
