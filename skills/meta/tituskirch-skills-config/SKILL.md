...
- **`docs`** — `preset` from project type (cli / package / app / infra), or `false` to opt out; language inherits root. `docs.instructions` for repo-wide docs wording, only on an explicit preference.
