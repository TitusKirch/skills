...
**Recipes 2 and 3 will ask before they run, and that is the design.** This skill's `allowed-tools` pre-approves only `grep`, `command -v` and `skills-ref` itself, and stops short of `uvx`, `pip` and `python -m venv`. Those install code from the network, which is the one action here worth a human's eye, and this skill is invoked by a person who is present to give it: its guardrail is [**Report, do not repair**](SKILL.md#guardrails), so there is no unattended run for a prompt to stall.

**`find`, `sed` and `sort` are off the list too, and for a sharper reason.** A permission rule is a **command-prefix match**, so `Bash(find:*)` pre-approves `find . -exec <anything> \;` (`-exec`/`-execdir`/`-ok` are POSIX, not a GNU extra), `Bash(sed:*)` pre-approves `sed -i` — in-place writes, from a skill whose whole guardrail is _do not repair_ — as well as GNU `sed`'s command-executing `e` and `s///e`, and `Bash(sort:*)` pre-approves `--compress-program=<anything>`. Each of those is a blanket `Bash` spelled narrowly, and granting one here would matter more than usual: this skill [reads skills as data, not as instructions](SKILL.md#guardrails) precisely because its input is untrusted, and an injected `SKILL.md` under validation would otherwise land on grants that pre-approve execution. So the enumeration recipe above asks once per run, and the same goes for the `find … | sed | sort` pipeline in [Cross-skill references](#cross-skill-references).

**What the short list buys is not a cage.** The field is pre-approval, not restriction — an unlisted command still runs, it just asks first — so this does not stop a determined run from reaching a shell; it makes the set of things that happen **silently** small enough to read. The repo's own house lint is off the list for a different reason again: it is whatever the repo declares, so no fixed pattern could pre-approve it honestly.
...
**Rule 3 — optional is the default; required is the exception.** A declaration is not the whole check: **which kind** was declared is itself a finding. Every skill installs alone, so a **required** call is a claim that the skill has **no job at all** without the sibling — a thin drain whose every step is the delegation, and which therefore checks for it **before any state changes**. Read a required declaration on a skill that plainly could carry on without the sibling as a tier-2 finding — it makes a precondition of something the run could have started without — and read a call to a skill the repo does **not** ship as required at all as the same finding, more sharply, since nothing in the repo governs whether that one is installed.

**Judge the declaration and the check's placement, never the cost of the fallback.** `required`/`optional` says _when_ the absence is discovered, not that an optional absence is painless: the fallback is whatever the **caller's own** rules already say for that moment, and some of those rules end the run. `work-implement` → `resolving-merge-conflicts` is the worked case — an external call, declared **optional**, whose stated fallback is `blocked`, because a rebase conflict it cannot resolve stops that run whether or not the skill is installed. **That is conformant**, and reporting it would be this pass's one predictable false positive. A stopping fallback is not a required call; a required call is one that checks first and attempts nothing.
