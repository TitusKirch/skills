...
The spec rule catalog and its tier assignment, the Claude Code extension set and the one re-tiered `skills-ref` line, discovering skills, getting and running `skills-ref` (install, invocation, exit codes, output), the unavailable-tool handling, the house-style tier — cross-skill references included — with this repo as the worked example, and where `skill-creator` fits: [REFERENCE.md](REFERENCE.md).
