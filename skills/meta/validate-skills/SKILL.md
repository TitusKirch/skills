...
- **The kind defaults to optional** — **required** is legitimate only where the skill has no job at all without the sibling (a thin drain whose every step is the delegation, checked before any state changes). A required call from a skill that could plainly carry on — and any required call to a skill the repo does not itself ship — is a finding: it turns an uninstalled optional skill into an aborted run.
