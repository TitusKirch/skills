...
- **`work`** — only when the repo runs the queue. Pick the backend (above); it and `linear.team` fall back to `issue.*`. Set `cap` (default 10), `branch` (`worktree` default), and the lifecycle `labels`. On Linear, also offer `linear.states` — the lifecycle step → workflow state map. It has no default (state names are per-team), and without it the worker moves the label but never the board, so propose it from the team's real states rather than leaving it out silently.
