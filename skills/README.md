...
- **`disallowed-tools`** _(optional)_ **[Claude Code]** — the field that **actually restricts**: tools **removed from Claude's pool** while the skill is active (cleared on your next message). This is the real "keep the skill away from X" control — e.g. `AskUserQuestion` on an unattended queue skill, where a question nobody answers would hang the run. Where a tool is named in both, `disallowed-tools` wins. Not part of the open standard.
  OpenCode ignores both `allowed-tools` and `disallowed-tools`; configure its effective controls in `opencode.json` or an agent's `permission` block. The two queue skills ship OpenCode agent templates that deny `question` for unattended runs.
...
Every skill with a `REFERENCE.md` ends on a pointer to it, and that pointer is **not a table of contents**. It states the **occasion** that sends an agent to the file — the step, the branch, the condition or the question — because the pointer's _wording_, not its target, decides whether the file is opened at the moment it was needed. A contents list leaves the agent to infer the occasion from a noun phrase, and the failure is silent: the file is present, correct, and unread.

- **Contents** — "Config keys, the merge modes, the `gh` recipes, the assessment checklist."
- **Occasion** — "Open it at step 3 for the assessment checklist and the two-bases problem, which is why a green check on the request as filed is not the evidence it looks like."

Naming what is inside stays useful; the occasion is what comes first, and what has to be there at all. Two rules keep that honest:

- **Reviewed, not tested — deliberately.** These pointers are hand-written and different per skill, so an assertion could only look for a word like "when", and _"read it when you need the reference"_ would pass it. That fakes completion, which is worse than the contents list it replaced. The mirrored blocks are pinned literally in [`test/isolation.test.ts`](../test/isolation.test.ts) because they are **generated**; a hand-written line is a review's to judge.
- **A pointer may never promise what the file no longer holds.** When material leaves — the rationale sections that moved to [`docs/99.adr/`](../docs/99.adr/) are the case — the pointer goes with it. A dangling promise costs more than a thin one: the agent opens the file and spends the whole read before finding the section gone.
...
- **Branch files are named from `SKILL.md`**, so they sit on level 1 beside `REFERENCE.md` rather than a hop behind it, and the two-levels rule above holds unchanged. Their pointer follows [the reference pointer](#the-reference-pointer) rule with the axis as its occasion: it states **the branch that reaches the file**, not what the file contains.
