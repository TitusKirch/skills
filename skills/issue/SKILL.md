...
- **Labels / state / team** — pick **from the cached catalog**, contextually for this issue, not from static defaults. **Skip any label matching `issue.labels.exclude`** (glob, e.g. `stack:*`, `autorelease:*`), and steer clear of clearly machine-managed labels (release-please / dependabot) even when unlisted. Linear needs a `team`: resolve `issue.linear.team` (a human name/key) to its id via the cache.
