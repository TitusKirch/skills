...
Mechanics for the [SKILL.md](SKILL.md) workflow. One skill, four trackers — GitHub `gh`, GitLab `glab`, Linear MCP, local files — chosen per-repo by config, one recipe each under `trackers/`, of which a repo reads exactly one. Which **host** a forge-native tracker talks to is resolved per repo too — [The forge and its host](#the-forge-and-its-host).
...
| `issue.local.dir`                     | `local` issue directory — repo-relative, default `.agents/issues` (`trackers/local.md`); the only key that tracker takes, and it has a default                                                                             |
...
| `issue.linear.priority`               | seeds Linear's **native priority field** on create — `none`/`low`/`medium`/`high`/`urgent`, mapped to the number the call takes (`trackers/linear.md`). Never a label, and never applied on GitHub                         |
