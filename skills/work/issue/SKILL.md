...
- **Labels / state / team** — a template's `labels:` and `assignees:` are the repo's own declaration; only **add** what they don't cover, from the **cached catalog** and contextually for this issue. Skip anything matching `issue.labels.exclude`. Linear needs a `team`. **One narrow exception:** the untriaged marker `work.labels.needsTriage` is **withheld** when the filing author is an [authorized](REFERENCE.md#author-authority) maintainer — [the untriaged marker](REFERENCE.md#the-untriaged-marker-is-not-for-an-author-who-would-triage-it).
