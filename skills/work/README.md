...
- **[decide](./decide/SKILL.md)** — Closes the open decisions a run already named by asking each one as a select dialog.
- **[handoff](./handoff/SKILL.md)** — Hands off in-progress work to another agent or session via a committed handoff document.
- **[issue](./issue/SKILL.md)** — Creates/updates/searches issues across GitHub (gh), GitLab (glab), Linear (MCP) or local files, tracker chosen by config.
