...
| `work.verify`                               | **the repo's own check command** (tests/lint/build) — a repo-level fact, not this skill's step; `null` → detect from the repo. This loop runs it before pushing; [`update-deps`](../../repo/update-deps/REFERENCE.md#config) and [`mergeDeps.verify`](../../repo/merge-deps/REFERENCE.md#config) run the same command at their own moments. Readers **outside** the `work-*` skills treat `work: false` as "not set" and fall back to detection — it disables the work loops, it does not claim the repo has no checks. |
