...
**Config key vs label string.** The states are keyed by their config names — `review`, `reviewing`, `changesRequested` — while the label _strings_ default to `ai: review requested`, `ai: reviewing`, `ai: changes requested`. This file names states by their **config key**; the human-facing diagrams below use the readable label. The `review` key's default string was renamed `ai: review` → `ai: review requested` so it reads symmetrically with `ai: changes requested`; **the key stayed `review`**, so a repo may pin `review: "ai: review"` and keep working unchanged.
...
| `work.labels.review`                        | the "pushed, awaiting AI review" hand-off label; default `ai: review requested` (was `ai: review`) — **key unchanged** |
...
- **`review` = awaiting AI review** by a **different** agent — not "awaiting a human". The `work-review` loop consumes it and writes the verdict. Its default label reads `ai: review requested` (renamed from `ai: review`).
...
**Migrate the review rename and the lease the same way.** The `review` default string was renamed `ai: review` → `ai: review requested`, and `reviewing` is new — both the **same class of changeover** as the loose locks above, where a primitive living under two names at once splits the very set it should partition, so the tracker and the config must move **before** the skill copies do. **The `work.labels.review` string must exist on the tracker before the new default takes effect:** a new copy writes and queries `ai: review requested`, and `gh issue list --label 'ai: review requested'` **exits 0** on an empty result, so a queue split between an old copy's `ai: review` and a new copy's `ai: review requested` stalls **silently**, with no error to notice. So before updating any skill copy, **pin `review: "ai: review"`** (the escape hatch that holds the old string in force) **or** create the `ai: review requested` label and relabel every open `ai: review` issue onto it — the pin covers the **steady** state, the relabel the **transition**. And **do not enable `reviewing` until every drain runs the new copy:** an old-copy review drain does not know the lease, so it selects the same issue straight off `review`, writes a **competing verdict**, and never reclaims a `reviewing` orphan invisible to it — the lease buys nothing until the last old copy exits (the same residual window the lock note reasons through), and enabling it mid-rollout is worse than leaving it off.
