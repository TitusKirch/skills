...
**Neither reconcile corrects it.** Neither sweep [**inspects**](#reconcile) a workflow state — the only state either one writes is the `states.ready` that accompanies a reclaim it is already performing — so a drifted state on an issue nothing is reclaiming is outside what either looks at. A `states` mapping therefore describes the board from its write until the first pull-request event, and nothing after.
...
Both move the issue's **lifecycle only** — label and assignee, plus on Linear the `states.ready` that accompanies a pre-push reclaim — never branches, and are **idempotent**: nothing to reclaim is the normal result. Their limit is what they never **read**: neither sweep **inspects** a workflow state, and the only state either writes is the one carried by a reclaim it is already performing. So a state [a second writer moved](#the-board-has-a-second-writer) on an issue neither sweep is reclaiming is not something either looks at, let alone puts back.
