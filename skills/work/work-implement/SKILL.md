...
- **Every optional call carries a stated fallback.** `atomic-commit`, `pull-request` and `resolving-merge-conflicts` are **optional** calls, not preconditions: without them, commit in the repo's own conventions, open the PR with the forge CLI, and treat an unresolvable rebase conflict as `blocked`. Verified work is never left uncommitted or unpushed because a helper skill is absent — the push is what the lifecycle turns on — and whichever fallback a run took is **named in its report**, never left silent. One rule, one place: [REFERENCE.md](REFERENCE.md#optional-skill-calls).
