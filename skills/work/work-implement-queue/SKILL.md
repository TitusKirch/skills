...
**A worker's reasoning effort is the session's, and this skill does not set it.** The Agent tool takes a per-spawn `model` and no `effort`, so every worker runs at whatever effort the session that started the drain is on. Implementing wants it **higher than reviewing does** — a weak pass costs a full review round — and because the two loops take separate locks they already run in separate sessions, which is where the setting belongs. Recommendation per loop, and why it is not pinned in frontmatter: **Worker effort** in `work-implement`'s REFERENCE.
