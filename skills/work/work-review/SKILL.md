...
**Two things decide what that wait reads, and both are easy to get subtly wrong.** The budget starts at the **later** of the un-draft and this head's own commit — a base that exists on _every_ round, including one answering a CI failure on a PR that is already ready, where nothing un-drafts and the un-draft event alone would put the deadline in the past before the first poll. And only checks whose run **started after** that moment count: a repo gating its jobs on the draft state reports them as `skipping` rather than not at all, so reading those stale rows as passes would accept a head CI never analysed.
