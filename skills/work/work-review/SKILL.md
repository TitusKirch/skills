...
| **`changes-requested`** | fixable problems, round < `maxRounds`                                                                          | **post the feedback** at the configured destination (`gh pr review --request-changes` with inline comments, or an issue/Linear comment referencing the commit), then set `changes-requested` — back to the implement loop |
| **`blocked`**           | broken beyond a fixable change / a hard human call                                                             | set `blocked` + comment                                                                                                                                                                                                   |

**The label move always lands on the issue; the comment goes where `work.feedback` says** — the PR's thread (`pr`) or the issue's comments (`issue`), so a three-round review does not turn the requirement into a log ([recipes](REFERENCE.md#feedback-recipes)). With no PR to write to, `pr` mode falls back to the issue and the run report names the fallback.
