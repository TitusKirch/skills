...
**A `done` is the moment CI is worth spending, so it is spent before the verdict is written.** Under `worktree` the implement loop leaves every PR a **draft** ([the draft gate](REFERENCE.md#marking-ready-then-waiting-for-ci)), which on a repo gating its workflows on that means no CI has run. So where the review above lands on `done` and the PR is still a draft: **mark it ready for review**, then wait for the checks the base actually triggers, bounded by `work.review.timeout` (default 600s).
...
- **Marking a PR ready for review is this loop's, and only on an accept.** It changes no code and merges nothing — it is the verdict said in the forge's own vocabulary, which is why the read-only rule above does not forbid it. Flip it only where the review would otherwise write `done`, **never** as housekeeping, and **never back into draft** on any verdict, CI failures included ([the draft gate](REFERENCE.md#marking-ready-then-waiting-for-ci)).
