---
name: update-deps
summary: Updates a repo's dependencies to their newest allowed versions — minor by default — across Node and PHP, driving the repo's own updater instead of bypassing its gates, pins and constraints.
description: Updates a repo's dependencies across Node (npm / pnpm / bun) and PHP (Composer), monorepos included — minor by default, with patch or major only when explicitly asked. Detects ecosystems from lockfiles and drives the repo's own updater (taze when present, else the native one), so a release-age gate, an exact pin or a declared constraint is honoured rather than bypassed, and whatever is held back is reported with its reason instead of silently passing. Always runs the advisory check, and a vulnerable dependency whose fix falls outside the run's range is reported loudly rather than skipped. Plans first and writes only after confirmation; never commits, pushes, opens a pull request or merges. Invoke manually only — this skill never fires proactively. Use when the user wants to update, upgrade or refresh dependencies, bump packages to newer versions, run taze or composer update, check for outdated packages or dependency advisories, or says things like "update the deps", "upgrade the packages", "Abhängigkeiten aktualisieren", "Pakete updaten".
allowed-tools:
  - Bash
  - Read
  - Grep
...
- **Ecosystems, from lockfiles** — `pnpm-lock.yaml` → pnpm, `bun.lock`/`bun.lockb` → bun, `package-lock.json` → npm, `composer.lock`/`composer.json` → Composer. Full table: [REFERENCE.md](REFERENCE.md#detection).
...
- **No taze → native** — `pnpm update`, `npm update`, `bun update`, `composer update`. These move **within the declared ranges** only; what that does and does not mean per ecosystem: [REFERENCE.md](REFERENCE.md#range--command).
...
Run the advisory check **every time**, even on a `patch` run, even when nothing else moves — `pnpm audit` / `npm audit` / `bun audit` / `composer audit`.
