...
**Opted out?** If the repo config sets `work` to `false`, all `work-*` skills are **disabled** — stop and tell the user they are turned off in `.tituskirch-skills.json`. Check `jq -e '.work == false'` before any action.
...
- **`review`** → review the pushed work (steps 4–6). The normal path.
