...
| Adding a new `<unit>` | The numbered path from empty folder to commit, ending at the commit message                          | A duplicate of the unit's own README or spec, or a list another file owns — the artifacts a sync command regenerates, a frontmatter contract, a hook's globs — least of all a counted one |
...
**It also drifts fastest, because it is the densest with repo internals.** Every step here names something another file owns, so it is where [never transcribe a file](SKILL.md#style-rules) is hardest to hold and costliest to break: name the sync command, not the artifacts it regenerates; name the file the frontmatter contract lives in, not the fields. A step that says how **many** of anything has invented the one claim no gate can check — and the naive repair, correcting the number, restores the defect the next time the owner grows a row.
