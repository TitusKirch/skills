...
1. **Resolve the preset** — `docs.preset` config → detect from the repo (bin/CLI → `cli`, a published manifest — library, module, agent/skill set → `package`, app/server → `app`, IaC → `infra`) → ask. Preset = which sections beyond the core.
