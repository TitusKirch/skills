...
Mechanics for the [`update-deps`](SKILL.md) skill. Scope is **Node** (npm / pnpm / bun), **PHP** (Composer), **Rust** (Cargo), **Go** (modules), **container images** (Dockerfile / Compose) and **GitHub Actions references** (workflows), monorepos included. The updater is **detected from the repo**, never configured — see [Decisions](#decisions). The last two are the ecosystems with no updater to detect, so the skill resolves and writes those itself — through a registry for [images](#container-images-version-model), through `gh api` for [action refs](#github-actions-version-model).
...
| Range               | taze (preferred) | pnpm / npm             | bun                   | Composer                        | Cargo                          | Go                      | Container images        | GitHub Actions          |
| :------------------ | :--------------- | :--------------------- | :-------------------- | :------------------------------ | :----------------------------- | :---------------------- | :---------------------- | :---------------------- |
| `patch`             | `taze patch -w`  | `pnpm update`¹         | `bun update`¹         | `composer update`¹              | `cargo update`¹                | `go get -u=patch ./...` | resolve + edit the tag³ | resolve + edit the ref⁴ |
| `minor` _(default)_ | `taze minor -w`  | `pnpm update`¹         | `bun update`¹         | `composer update`¹              | `cargo update`¹                | `go get -u ./...`       | resolve + edit the tag³ | resolve + edit the ref⁴ |
| `major`             | `taze major -w`  | `pnpm update --latest` | `bun update --latest` | `composer require <pkg>:^<new>` | `cargo upgrade --incompatible` | **report and stop**²    | a base-image change³    | `@v4` → `@v5`⁴          |
...
- **Resolve through `gh api`**, against the host the repo resolves to — the root `forgeHost` key where the config states one, else the host in the `origin` remote, else whatever `gh` is already configured for. Pass it as `GH_HOST=<host> gh api …` where it was resolved, and pass nothing where it was not. `gh api repos/<owner>/<repo>/tags` lists the tags a ref may move to; `gh api repos/<owner>/<repo>/git/ref/tags/<tag>` resolves one to the commit a SHA pin needs. `gh` is the CLI this skill's siblings already depend on, so this adds no tool the repo does not have.
...
- **Keep a shared dependency on one version across packages** — a version skew introduced by an update is a finding, not an outcome. A **shared base image** is the same rule: three apps on `FROM node:22` that come out on two different tags is a skew, however green each one is alone. So is a **shared action**: `actions/checkout` left on two different refs across a repo's workflows is a skew whichever ref is newer, and it is the ordinary way a workflow set drifts.
...
**An ecosystem with no scanner at all is the third case, and the report must not collapse it into either of the other two.** Nothing of `cargo audit`'s shape exists for [GitHub Actions references](#github-actions-version-model), so "install it" is not the fix and there is nothing to be missing. Report that section **not available for this ecosystem**, and put the exposure that _is_ knowable there instead: which refs the repo pins by **tag** rather than by SHA, named as the standing risk it is and left for a human to act on.
