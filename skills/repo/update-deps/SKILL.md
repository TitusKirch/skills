...
description: Updates a repo's dependencies across Node (npm / pnpm / bun), PHP (Composer), Rust (Cargo), Go (modules), container images (Dockerfile / Compose) and GitHub Actions (workflows), monorepos included — minor by default, patch or major only when explicitly asked. Detects ecosystems from lockfiles and manifests, driving the repo's own updater where one exists, so a release-age gate or an exact pin is honoured and what is held back is reported with its reason. Always runs the advisory check, reporting loudly what it cannot fix. Invoke manually only — plans first, writes after confirmation, never commits, pushes, opens a PR or merges. Use when the user wants to update, upgrade or refresh dependencies, bump packages, image tags or action refs, run taze, composer update, cargo update or go get, check for outdated packages or advisories, or says things like "update the deps", "upgrade the packages", "Abhängigkeiten aktualisieren", "Pakete updaten".
allowed-tools:
  - Bash
  - Read
  - Grep
...
- **An ecosystem with no scanner _at all_ is a third case, and it is not clean either.** No advisory command exists for GitHub Actions references — nothing of `cargo audit`'s shape has ever shipped for them — so that section is reported **not available for this ecosystem**, distinct from a tool that is merely not installed. What belongs there instead is the standing exposure: a repo pinning actions by **tag** rather than by SHA trusts a ref its author can re-point, and naming that is the report's job. Rewriting it is not — see the guardrails.
...
- **Unread** — an ecosystem the run could not resolve at all: an unreachable registry, a `gh` that was unauthenticated or rate-limited. Never folded into "nothing to update".
- **Advisories** — open ones, which are fixed by this run, which are not, and why not. A scanner that was **unavailable**, or an ecosystem with **no advisory command at all**, belongs here too, named as such rather than left out — with the tag-pinned action refs a repo is standing exposed on.
...
- **Never pin a floating reference, and never change a variant suffix.** `:latest`, `node:22`, `alpine:3` and a GitHub Actions `@v4` or `@main` ref already resolve to the newest thing in their track, so rewriting one narrows what the repo deliberately left open. `-alpine`, `-slim` and `-bookworm` are a base-OS choice, so `22-alpine → 24-slim` is a different image, not an update. Both are **reported**, never moved.
...
- **Never change a repo's pinning policy.** Rewriting tag refs to SHA pins is a hardening pass, not a dependency update, and the same holds for the reverse. Report the exposure and leave the decision to a human.
...
Detection table, the range → command mapping per ecosystem, the release-age gate (with a worked example), exact pins, Composer's constraint model, the container tag model, the action ref model, monorepos, and the reasoning behind the defaults: [REFERENCE.md](REFERENCE.md).
