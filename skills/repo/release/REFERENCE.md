...
Mechanics for the [`release`](SKILL.md) skill. **GitHub (`gh`) is the only forge this skill drives, by decision rather than by staging** — the binding is the **release tool**, not the forge, and it is settled under [GitHub-only](#github-only-and-why-the-release-tool-binds-it). The release tool it drives is **release-please**, detected from the repo rather than configured — see [Decisions](#decisions).

## GitHub-only, and why the release tool binds it

The root `forge` key accepts `github` and `gitlab` ([ADR-0028](https://github.com/TitusKirch/skills/blob/main/docs/99.adr/0028-dock-gitlab-and-resolve-the-host-per-repo.md)); **this skill drives only `github`**, and meeting any other value it stops rather than degrading. That is a **settled decision**, not a gap awaiting a driver, and the stop message says so — "not supported yet" would promise work nobody is doing.

**The forge is not what binds it. The release tool is.** This skill drives a _model_ — promote, wait for the release PR the tool opens, validate it, merge it, let the tool tag — and that model needs a tool that opens a release PR:

| Candidate                       | Why it does not unbind this skill                                                                                                                                                                      |
| :------------------------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **release-please**, on GitLab   | GitHub-only **by construction** — its integration with the GitHub API is deep enough that no other forge has ever been attempted. GitLab's own request for it (`gitlab-org/gitlab#387240`) is a wish.  |
| **semantic-release**, on GitLab | The one with real GitLab traction, and the wrong **shape**: it tags straight from the pipeline with no release MR at all, so the validate-and-merge core here would have no object to act on.          |
| **GitLab `release-cli`**        | Creates releases; does not version and does not write a changelog. A different model, not a driver swap.                                                                                               |
| **`releaser-pleaser`**          | Reproduces the release-PR model on **both** forges, so a GitLab driver is technically reachable — at the cost of a second supported detection target, small user base, for a forge no repo here is on. |

So the limitation is **reachable and not worth reaching**, which is a different statement from "unimplemented", and it is the one a GitLab repo is owed up front rather than on the run that stops.

**Nothing here closes the door.** The two seams that would carry a second tool are deliberately untouched: the release tool is [detected, not configured](#decisions), so a new one arrives as detection plus a branch in the workflow rather than as a config break; and the skill's **name** is the tool-neutral noun for the same reason. A repo on GitLab that wants a release flow drives its own — this skill says so and gets out of the way.
