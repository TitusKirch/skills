...
Mechanics for the [`release`](SKILL.md) skill. **GitHub (`gh`) is the only forge this skill drives, by decision rather than by staging** — the binding is the **release tool**, not the forge, and it is settled under [GitHub-only](#github-only-and-why-the-release-tool-binds-it). The release tool it drives is **release-please**, detected from the repo rather than configured: `release-please-config.json` and the workflow already say which tool the repo runs, so a `release.tool` key could only let the config contradict the repo.
...
**Nothing here closes the door.** The two seams that would carry a second tool are deliberately untouched: the release tool is **detected, not configured**, so a new one arrives as detection plus a branch in the workflow rather than as a config break; and the skill's **name** is the tool-neutral noun for the same reason. A repo on GitLab that wants a release flow drives its own — this skill says so and gets out of the way.
