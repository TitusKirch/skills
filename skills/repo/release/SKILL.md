...
| `"create"`          | No such automation — the skill may open the PR itself, via `pull-request` with the edge's `base` and `head`. **The only PR this skill ever opens, in any mode.**                                    |
