...
**This skill owns no config section.** Every knob it could have — which languages, how wide the scope, how aggressive to be — is a per-run decision, and there is no unattended act to disable: the skill plans first, writes after confirmation, and never commits or pushes, so **not invoking it** is the off switch. That covers the knobs; it does not cover the **facts**. It therefore reads three keys other sections already own, each a standing fact about the **repo** rather than about this run:
...
**And why the check command will ask before it runs.** This skill's `allowed-tools` names the commands it drives rather than granting `Bash` outright: `jq` and `printf` for the config, the four **read-only** git calls the scoping below uses (`git diff`, `git ls-files`, `git rev-parse`, `git symbolic-ref --short`), and `head` / `grep` for reading a file's first lines. `git symbolic-ref` is granted at `--short` rather than at the bare subcommand, because the two-argument form of it repoints a ref and this skill only ever prints one. The repo's `verify` is deliberately absent, because it is _whatever the repo declares_ — no fixed pattern can pre-approve an arbitrary command without pre-approving every command. So it prompts once, which costs nothing here: this skill [confirms with a human before it removes anything](SKILL.md#guardrails), so a person is already present, and the field is pre-approval rather than restriction — an unlisted command still runs, it just asks first.

**`sh` and `sed` are absent for a second reason.** A permission rule is a **command-prefix match**, so `Bash(sh:*)` would pre-approve `sh -c '<anything>'` — including the very `git commit`, `git push` and `git checkout` the four read-only calls are chosen to leave out — and `Bash(sed:*)` would pre-approve `sed -i`, an in-place file write from a skill that edits comments and nothing else. Either one is a blanket `Bash` spelled narrowly. The cost is that running [`resolve-config.sh`](templates/resolve-config.sh) asks once, the same as `verify`; the trade is that the scoped list stops describing a boundary it does not hold.

**What the list is, and is not.** It documents what this skill drives and keeps the unattended surface small; it is **not** a restriction, so nothing here _prevents_ a commit or a push — an unlisted command still runs once a person says yes. That match between the list and the guardrail _never commit, push, open a PR or merge_ is the point: what the skill promises in prose and what it takes without asking now say the same thing.
