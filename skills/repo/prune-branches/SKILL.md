...
- **Remote** — the integration branch's own remote (`git config branch.<base>.remote`), else `origin`. **One remote per run.** Forks, mirrors and any second remote are neither read nor written — a fork or a mirror is somebody else's copy, and a branch name matching across two remotes is a coincidence rather than a relationship.
...
- **`release/*` is deliberately not protected** — that is the shape of release-please's own short-lived branches, which are exactly what wants pruning. A repo that genuinely ships from `release/*` adds it via `pruneBranches.protect`; the additive key exists for precisely this.
