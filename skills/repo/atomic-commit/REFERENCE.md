...
lang=$(printf '%s' "$resolved" | jq -er '.commit.language // .language // empty' 2>/dev/null) || lang=
scopes=$(printf '%s' "$resolved" | jq -er '.commit.scopes // empty' 2>/dev/null) || scopes=          # true|false|auto
scope_vocab_cfg=$(printf '%s' "$resolved" | jq -er '(.commit.scopeVocab // []) | join(" ")' 2>/dev/null) || scope_vocab_cfg=
instructions=$(printf '%s' "$resolved" | jq -er '.commit.instructions // empty' 2>/dev/null) || instructions=
```
...
- **Dependency bumps, by release relevance.** A **runtime** dependency that should reach users → `fix` (release-please's own maintainers do exactly this: `fix: bump deps for security alerts`). A **dev-/CI-only** bump → `chore` (intentionally no release). Avoid `feat(deps)` — it over-inflates the minor; `build(deps)`/`chore(deps)` ship nothing, which is correct only for dev/CI deps.
- **Prod may be gated on the release.** A `deploy-prod` that runs only on `release_created` never sees a non-releasable type, so a shipping change mis-typed `chore`/`refactor` never reaches production — one more reason to bias `feat`/`fix`.
