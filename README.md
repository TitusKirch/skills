...
| [`handoff`](skills/handoff/SKILL.md)                                   | Hands in-progress work to another agent, session or machine through a structured handoff document committed to the repo.                                                                            |
| [`issue`](skills/issue/SKILL.md)                                       | Creates/updates/searches issues across GitHub (gh) or Linear (MCP), tracker chosen by config.                                                                                                       |
| [`merge-deps`](skills/merge-deps/SKILL.md)                             | Triages the open Dependabot pull requests — verifies each one on its own branch, then merges what the repo's config allows.                                                                         |
...
| [`update-deps`](skills/update-deps/SKILL.md)                           | Updates a repo's dependencies to their newest allowed versions — minor by default — across Node, PHP and Rust, driving the repo's own updater instead of bypassing its gates, pins and constraints. |
